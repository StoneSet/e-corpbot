...
bot.on("guildMemberAdd", member => {
  let role = member.guild.roles.find(
    "name",
    "mettre le nom du role exact voulue"
  );
  member.guild.channel
    .find("name", "nom du channel ou le message va s'ouvrir")
    .send(
      `:heart: ${member.user.username} vient de rejoindre le serveur E-Corp !`
    );
  member.addRole(role);
});
...
bot.on("guildMemberRemove", member => {
  member.guild.channel
    .find("name", "nom du channel ou le message va s'ouvrir")
    .send(`:heart: ${member.user.username} vient de nous quitter :/ !`);
});
...
bot.on("message", message => {
  ...
  if (
    ...
    db
      .get("xp")
      .push({ user: msgauthor, xp: 13 })
      .write(); // nombre d'xp pour chaque message
  } else {
    var userxpdb = db
      .get("xp")
      .filter({ user: msgauthor })
      .find("xp")
      .value();
    ...
    db
      .get("xp")
      .find({ user: msgauthor })
      .assign({ user: msgauthor, xp: (userxp[1] += 13) })
      .write(); // "fonction", qui ++ les nb c'xp
  }
  ...
  if (message.content === prefix + "xpstat") {
    ...
    var xp_embed = new Discord.RichEmbed()
      ...
      .setDescription(
        "Voici le nombre d'XP que vous avez accumlez depuis votre arrivé sur le serveur E-Corp !"
      )
